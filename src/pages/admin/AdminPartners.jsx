...
export default function AdminPartners() {
  ...
  function openEdit(row) {
    setEditing(row)
    setForm({ name: row.name||'', order: row.order??0 })
  ...
  const columns = [
    { key: 'logo', label: 'لوگو',
      render: (v, row) => v
        ? <img src={row.logo_url || v} alt="" className="h-10 w-20 object-contain rounded-lg bg-gray-800 p-1"/>
  ...
  return (
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir="rtl">
          ...
          <div className="relative bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              <ImageUpload label="لوگوی شریک" value={imageFile ? URL.createObjectURL(imageFile) : editing?.logo_url} onChange={setImageFile}/>
              <div>
      ...
      <ConfirmModal open={!!deleteTarget} title="حذف شریک" message={`شریک "${deleteTarget?.name}" حذف شود؟`} onConfirm={handleDelete} onCancel={()=>setDeleteTarget(null)} loading={deleting}/>
    </div>
  )
}
