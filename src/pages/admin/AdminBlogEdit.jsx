...
export default function AdminBlogEdit({ id }) {
  const isNew = id === 'new'
  ...
  useEffect(() => {
    ...
    fetch(`${API}/${id}`)
      ...
      .then(record => {
        ...
        if (record.cover_url) setExistingCover(record.cover_url)
        setLoading(false)
      })
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => router.push('/admin/blog')} className="text-gray-400 hover:text-white transition-colors">
      ...
      <div className="space-y-5">
        <div>
          ...
          <input type="text" value={form.title} onChange={e => handleTitleChange(e.target.value)}
            className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
        </div>
        <div>
          ...
          <input type="text" value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))}
            className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono" dir="ltr"/>
        </div>
        <div>
          ...
          <textarea value={form.summary} onChange={e => setForm(p => ({ ...p, summary: e.target.value }))} rows={3}
            className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"/>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1.5">محتوا (HTML)</label>
          <textarea value={form.content} onChange={e => setForm(p => ({ ...p, content: e.target.value }))} rows={14}
            className="w-full bg-gray-900 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y font-mono" dir="rtl"/>
        </div>
        <ImageUpload label="تصویر کاور"
      ...
      <div className="flex gap-3 justify-end mt-8">
        <button onClick={() => router.push('/admin/blog')} disabled={saving}
          className="px-5 py-2.5 text-sm text-gray-300 bg-gray-800 hover:bg-gray-700 rounded-xl transition-colors">انصراف</button>
